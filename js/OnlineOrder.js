...
var ApiDomainURL = "https://webapi-v1.marnpos.com/api/"; //"http://localhost:31607/api/";	 //"http://app.isalespos.com/api/"; // "http://localhost:3762/api/";//http://app.isalespos.com/api/";
    ...
    GetAPI(ApiDomainURL + "locationmp/" + sessionStorage.getItem("lics")).done(function (data) {
        ...
        if (data.Status === 1) {
            ...
            $.each(data.LocationsList, function (i, res) {
				...
				//var SessionID = res.SessionID;  //TEST Session: "POS-87RCHB786201708210725007534";//REAL BURGER : "POS-RK5SFX786201708100912178213";    //res.SessionID;
                var SessionID = "POS-87RCHB786201708210725007534";



                locationsView += ' <div><div id="map-' + res.LocationID + '" class="map-box" address="' + res.Address + '" lat="' + eval(24.708241).toString() + '" lng="' + eval(46.773526).toString() + '"></div><img src="../images/map.png"  lat="' + eval(24.708241).toString() + '" lng="' + eval(46.773526).toString() + '" style="display:none" minOrderAmount="' + MinOrderAmount + '"  deliveryTime="' + DeliveryTime + '" deliveryCharges="' + DeliveryCharges + '" class="images-location-" id="' + i + '" locationid="' + res.LocationID + '" address="' + res.Address + '" sessionId="' + SessionID + '"  location="' + res.Name + '" /><h3>' + res.Name + '</h3></div>';
            ...
            $.when.apply($, arr).done(
                function () {
                    ...
                    $('#waterwheelcarousel .map-box').click(function () {
                        ...
                        $(this).next().attr('src', "../images/select-map.png");
        ...
        $.when.apply($, arr).then(
            ...
            function () {
                ...
                objData = {
                    ...
                    OfflineOrderID: "",
                    OrderNo: 0,
                    OrderStatus: 301, //For Web
                    OrderTakerID: "1",
                    OrderType: 'Web', //objWebOrder.OrderType,// "Walk in",
                    ReturnID: 0,
        ...
        if (currentItem !== null) {
            ...
            $(_this).parent().parent().parent().parent().find('.modifiersRows').each(function (index, value) {
                if ($(this).find('input[type="checkbox"]').prop("checked") === true) {
                    ...
                    _items += '<div class="row row-modifiers" ModifierId="' + ModifierId + '" modifierprice="' + modifierprice + '" Name="' + Name + '" itemid="' + itemid + '"><div class="two wide column "></div><div class="seven wide column modifier-name"><img src="../images/delete-icon-gray.png" onclick="modifierDelete(this,' + currentItem.index + ',' + ModifierId + ')" style="width: 10px;height: 10px; margin-right:5px;" />' + Name + '</div><div class="four wide column modifier-qty">' + Qty + '</div><div class="three wide column price total">' + Total + '</div></div>';
...
function customAccordian(_this, currentVisibleElement) {
	...
	//alert(objWebOrder.OrderType.toLowerCase());
	if (objWebOrder.OrderType.toLowerCase() == "pickup") {
		$("#DeliveryAddressGuest").attr("style", "display:none");
    }
}
        ...
        $(_this).each(function (index, value) {
            ...
            if (tabname == 'step2') {
                ...
                if (sessionStorage.getItem("locationId") == undefined || sessionStorage.getItem("locationId") == null) {
					...
					$("#eleven").attr("class","eleven wide column");
					$("#five").removeAttr("style");
					$("#mainRow").attr("class", "ui two column stackable grid");
            ...
            if (tabname == 'step3') {
                ...
                $('.ui.form.RegisterYourself').form(formValidationRulesRegisterCustomer_yourself, RegisterCustomer);
				

				if (objWebOrder.OrderType.toLowerCase() == "pickup") {
					$('.ui.form.GuestOrder').form(formValidationRulesRegisterCustomer_Pickup, GuestOrderRegister);
				}
				else {
...
function imgError(image) {
    ...
    image.src = "../images/default_thumb_image.png";
    //image.src = "../images/defaultCategoryIcon.png";
            ...
            if (data.Category !== null) {
                ...
                if (data.Category.length > 0) {
					...
					if (data.Category.length <= 1) {
						$("#ddlCategory").attr("style", "display:none");
					}
					else {
         ...
         function (ind) {
             ...
             var _items = '<div class="ui grid pnl-items" rowid=' + RowItemNo + ' index = ' + ObjectOrderDetail.index + ' item-price=' + ObjectOrderDetail.price + ' name="' + ObjectOrderDetail.name + '" itemid=' + ObjectOrderDetail.itemId + '>' +
                                                     '<div class="row row-items">' +
                                                         '<div class="two wide column delete" onclick="itemDelete(this)"><img src="../images/delete%20icon.png"  style="margin-left: 10px;" /></div>' +
		...
		onSuccess: function () {
			...
			ReturningCustomerLoginApi(function (response) {
				...
				if (parseInt(response.Status) === 1) {
					...
					var ObjCustomer = response.Customer[0];
					ServerCustomerId = ObjCustomer.CustomerID;
					objWebOrder.CustomerID = ObjCustomer.CustomerID;
					// ChangeStep('step-5', '1');
...
function CustomerRegisterApi(callback) {
	...
	//////Register Customer Code from new API//////
	var gender = document.getElementById("txtGender").options[document.getElementById("txtGender").selectedIndex].value;
	var _city = document.getElementById("txtReg_City").options[document.getElementById("txtReg_City").selectedIndex].value;
	...
	var _note = "-";
	var _holddays = 0;
	var _country = (document.getElementById("txtReg_Country").value === "") ? "Saudia" : document.getElementById("txtReg_Country").value;
	_city = (_city === "") ? "Riyadh" : _city;
	var _area = "0";
	var _zip = "0";
	var _password = document.getElementById("txtPassword").value;

	var url = "" + ApiDomainURL + "/customerRegister/";
	...
	PostApiData(url, _data, function (response) {
		if (response.Status === 1) {
            ...
            //Code Here for customerdetail
			console.log(response);
			if (parseInt(response.Status) === 1) {

			    var url = "" + ApiDomainURL + "/customerDetail/";
				var _cdata =
					{
						"CustomerID": objWebOrder.CustomerID,
						"Address": $("#txtReg_Address").val(),
						"City": $("#txtReg_City").val(),
						"Country": $("#txtReg_Country").val(),
						"ContactNo": $("#txtContact").val(),
						"POBOX": $("#txtReg_PostalCode").val(),
						"Latitude": currLat,
		                "Longitude": currLng,    
						"LastAddress": "-",
						"SessionId": sessionStorage.getItem("session")
				...
				PostApiData(url, _cdata, function (responseCD) {
					console.log(responseCD);
					if (responseCD.Status === 1) {
						var CustomerDetailID = responseCD.CustomerDetailID;

						//alert(responseCD.Description);
						callback(responseCD);
			        }
					else {
						callback(responseCD);
...
function ReturningCustomerLoginApi(callback) {
	...
	var userid = document.getElementById("txtRetCusMobile").value;	//"as@sa@mailc.om";
	var password = document.getElementById("txtRetCusPwd").value;	//"Test@123";

	var url = "" + ApiDomainURL + "/Login?userid=" + userid + "&password=" + password+"";
...
var formValidationRulesRegisterCustomer_yourself = {
	on: 'blur',
	cname: { identifier: 'cname', rules: [{ type: 'empty', }] },
	cemail: { identifier: 'cemail', rules: [{ type: 'email', }] },
	ccontact: { identifier: 'ccontact', rules: [{ type: 'empty', }] },
	cpassword: { identifier: 'cpassword', rules: [{ type: 'empty', }] },
	cgender: { identifier: 'cgender', rules: [{ type: 'empty', }] },
	cbirthdate: { identifier: 'cbirthdate', rules: [{ type: 'empty', }] },
	ccountry: { identifier: 'ccountry', rules: [{ type: 'empty', }] },
	ccity: { identifier: 'ccity', rules: [{ type: 'empty', }] },
	cpostal: { identifier: 'cpostal', rules: [{ type: 'empty', }] },
...
var formValidationRulesRegisterCustomer_Pickup =
	{
		on: 'blur',
		name: { identifier: 'name', rules: [{ type: 'empty', }] },
		//email: { identifier: 'email', rules: [{ type: 'email', }] },
